...
class Stocker():
    ...
    def __init__(self, ticker):
        ...
        QUERY = ('SELECT * FROM `autotrader-test-1.PriceData.%s` LIMIT 1000' % ticker)

        query_job = client.query(QUERY)
        q = query_job.result()
        stock = q.to_dataframe()
        ...
        stock.rename(columns={stock.columns[1]:"Date"},inplace=True)
        data = []
        count = 0
        stock = stock.sort_values(by=['Date'])
        for i in stock['Date']:
            data.append(pd.Timestamp(i))
            count += 1
        ...
        self.changepoint_prior_scale = 0.3 
    ...
    def fifty_mv(self, current_ind):
        return self.stock['Adj. Close'].iloc[current_ind-50:current_ind].mean()
            
    def twohunned_mv(self, current_ind):
        return self.stock['Adj. Close'].iloc[current_ind-200:current_ind].mean()
    def moving_average_model(self, start_date=None, end_date=None, nshares = None):
        self.reset_plot()
        start_date, end_date = self.handle_dates(start_date, end_date)
        prevfif = 0
        prevtwo = 0
        ...
        fiflist = []
        twolist = []
        current_index = len(self.stock['Date'])
        datelist = self.stock['Date'].iloc[200:current_index]
        for i in range(200, current_index):
            fif = self.fifty_mv(i)
            twoh = self.twohunned_mv(i)
            fiflist.append(fif)
            twolist.append(twoh)
            if prevfif < prevtwo and fif > twoh:
                stockswitch = 1
                prevfif = fif
                prevtwo = twoh
            if prevfif > prevtwo and fif < twoh:
                ...
                prevtwo = twoh
            if stockswitch == 1:
                profit = self.stock['Adj. Close'][i] - self.stock['Adj. Open'][i]
                total += profit*nshares 
            profitlist.append(total)
        extralist = datelist
        plt.plot(datelist,profitlist)
        #plt.plot(extralist,fiflist)
        #plt.plot(extralist,twolist, linestyle='dashed')
        plt.show()
    ...
    def evaluate_prediction(self, start_date=None, end_date=None, nshares = None):
        ...
        if start_date is None:
            start_date = self.max_date - pd.DateOffset(years=3)
        if end_date is None:
            end_date = self.max_date
        ...
        # If a number of shares is specified, play the game
            ...
            plt.grid(alpha=0.2); 
            plt.show()
            result = [np.sum(prediction_profit), float(test.ix[len(test) - 1, 'hold_profit'])]
            return result
